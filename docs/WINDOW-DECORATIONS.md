...
3. native 최소화가 compositor 상태로 수렴하지 않는 관리 대상 Electron 앱과
   장식 없는 앱만 `window-interaction.yaml` positive allowlist의 Enoshima titlebar
...
positive allowlist는 XWayland의 Motif no-border hint보다 우선한다. 이 hint는
client가 외부 장식을 원하지 않는다는 일반적 신호지만, registry에서
`enoshima-system`으로 승인한 class는 장식 소유자를 이미 명시적으로 결정했기
때문이다. allowlist 밖의 X11 창에는 이 예외를 적용하지 않는다.
...
| Notion | `*notion*` | Electron/Wayland | Enoshima system titlebar | `notion-flags.conf`에서 Wayland client decoration 비활성화 | Electron native decoration으로 한 커밋 rollback | deterministic Electron VM 검증, 실제 앱 수동 UI 검증 필요 |
...
| Discord | `discord` | Electron/Wayland | Enoshima system titlebar | `discord-wayland`에서 Wayland client decoration 비활성화 | Electron native decoration으로 한 커밋 rollback | deterministic Electron VM 검증, 실제 앱 수동 UI 검증 필요 |
| Slack | `slack`, `com.slack.Slack` | Electron/Wayland | Enoshima system titlebar | `slack-wayland`에서 Wayland client decoration 비활성화 | Electron native decoration으로 한 커밋 rollback | deterministic Electron VM 검증, 실제 앱 수동 UI 검증 필요 |
| Obsidian | `obsidian`, `md.obsidian` | Electron/Wayland | Enoshima system titlebar | `obsidian/user-flags.conf`에서 Wayland client decoration 비활성화 | Electron native decoration으로 한 커밋 rollback | deterministic Electron VM 검증, 실제 앱 수동 UI 검증 필요 |
...
| RHWP Desktop | `rhwp*` | Electron/Wayland | Enoshima system titlebar | package wrapper에서 Wayland client decoration 비활성화 | Electron native decoration으로 한 커밋 rollback | deterministic Electron VM 검증, 실제 앱 수동 UI 검증 필요 |
...
client-owned Electron CSD 최소화 경로를 진단할 때는 별도 터미널에서 Hyprland의
문서화된 `minimized` 이벤트만 관찰한다. title이 포함되는 다른 이벤트는 기록하지
않는다. 2026-07-21의 disposable VM에서 Electron 39의
`BrowserWindow.minimize()`는 Wayland와 XWayland 모두 프로세스를 살려 둔 채
Hyprland workspace를 변경하지 않았다. 따라서 위 관리 대상 Electron 앱은 해당
불안정 경로를 사용자에게 노출하지 않고 Enoshima exact-address control을 사용한다.
...
현재 positive allowlist는 `mpv`, `imv`, `org.pwmt.zathura`와 관리 대상 Electron
class인 `discord`, `slack`, `com.slack.Slack`, `obsidian`, `md.obsidian`,
`*notion*`, `rhwp*`다. 공식 `hyprbars` fallback은 없다.

allowlist를 추가하려면 다음 정보를 같은 변경에 기록하고 focused test와
`scripts/validate.sh`를 통과해야 한다.

- application과 정확한 runtime class/backend
- native/system decoration이 없거나 native minimize가 Hyprland 상태로 수렴하지
  않는다는 재현 절차와 증거
- fallback을 선택한 이유와 검증 날짜
- managed launch flag가 client decoration을 비활성화한다는 증거와 class가
  client-owned registry에 없다는 검사
