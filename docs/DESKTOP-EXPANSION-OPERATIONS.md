...
2. `hyprbars-setup`으로 현재 Hyprland ABI에 맞는 공식 hyprbars를 준비한다.
3. `cloudflare-one-setup`으로 tray를 켜고 GUI에서 Zero Trust 등록을 마친다.
4. `rclone-cloud-setup all`로 Google Drive와 Proton Drive를 등록한다.
5. `protonmail-bridge-setup`에서 Bridge와 Thunderbird를 연결한다.
6. `kakaotalk-connectivity-check`와 `kakaotalk-setup`을 실행한다.
7. ONLYOFFICE, RHWP Desktop과 PhotoGIMP를 복사본으로 검증한다.
8. 전체 수동 acceptance를 통과한 뒤에만 SDDM cyberpunk theme를 활성화한다.
...
- Hyprpaper, Waybar, SwayNC, Hyprlauncher, Hyprlock, Dock과 titlebar의 색·글꼴
...
Hyprbars는 Hyprland ABI와 결합되므로 Hyprland 세션의 터미널에서 다음 helper를
대화식으로 실행한다.

```bash
hyprbars-setup
hyprbars-setup --check
```

`hyprbars-check.service`는 로그인 때 저장된 ABI와 현재 ABI를 비교한다. Hyprland
업그레이드 뒤 알림이 나타나거나 plugin load가 실패하면 `hyprbars-setup`을 다시
실행한다. keyboard fallback인 `Super+C` close는 plugin 실패 중에도 유지한다.

Acceptance에서는 다음을 확인한다.

- red는 close, yellow는 `cyberdock-minimize`, green은 work-area maximize/restore
- green double click과 `Super+F`는 true fullscreen
- tiled·floating client 모두 동작하며 client-side titlebar와 중복되지 않음
- 최소화된 창은 `special:minimized`에 있고 Dock에서 원래 workspace/output으로
  돌아옴

최소화 상태는 `$XDG_RUNTIME_DIR/cyberdock/`에만 저장된다. Dock이나 plugin 문제로
