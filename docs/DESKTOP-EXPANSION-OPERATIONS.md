...
Flatpak에 포함된 Wine 11 이상 runner, X11/XWayland, 144 DPI와
`XMODIFIERS=@im=fcitx`를 설정한다. KakaoTalk의 Themida 보호기와 충돌하는 Soda 9를
사용하지 않으며, 게임용 DXVK/VKD3D DLL을 Wine 내장 DLL로 복원해 `Wrong DLL
Present` 오류를 방지한다. 파일 권한은 Downloads, Documents, Pictures로 제한된다.
installer 동의, 로그인과 snapshot 생성은 대화식이다. update 전 Bottles에서
snapshot을 만들고 검증된 Wine 11 이상 runner를 유지한다.
...
KakaoTalk 호환 설정에서 Wine 11 이상으로 전환하고 DXVK·VKD3D는 제거한다.
...
`korean:ralt_hangul`을 적용한다. `kakaotalk.exe`에만 Wine XIM
`InputStyle=root`를 설정해 Fcitx가 조합 중 문자열을 표시하고 카카오톡에는 확정된
문자열만 전달하므로, 다른 앱의 preedit 동작을 바꾸지 않고 한글 입력 밀림을 피한다.
