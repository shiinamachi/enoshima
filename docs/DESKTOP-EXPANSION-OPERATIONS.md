...
X11/XWayland, 144 DPI와 `XMODIFIERS=@im=fcitx`를 설정한다. 숫자가 가장 큰
...
snapshot을 만들고 검증된 고정 runner를 유지한다. 프로필에 선언된 `cjkfonts`,
...
runner 변경이나 회귀 발생 시 `kakaotalk-profile rollback`으로 이전 로컬 선택과
