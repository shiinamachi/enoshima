...
- pacman 패키지, greetd/ReGreet, fallback SDDM, 시스템 서비스와 `/usr` 아래 파일은 Ansible 또는 로컬
  PKGBUILD가 소유한다.
...
- fingerprint-only 로그인에서는 GNOME Keyring이 잠긴 채일 수 있다. 계정
  온보딩과 첫 전체 검증은 ReGreet에서 비밀번호로 로그인한 세션에서 수행한다.
...
재부팅 후 ReGreet에서 `Enoshima Hyprland` 세션으로 비밀번호 로그인한다.
...
새 profile은 schema 2와 display policy revision을 함께 기록한다. 이전 기본값인
eDP 1.5/Dell 1.5 schema 1 profile은 `reconcile`에서 `balanced`로 한 번만
마이그레이션하며, 다른 배율이나 좌표가 있는 사용자 profile은 그대로 보존한다.
...
7. ReGreet 로그인과 SDDM 비상 rollback을 모두 수동 검증한다.

계정 등록 중 표시되는 계정명, 팀·조직 정보, 토큰과 생성된 메일 비밀번호를
문서, 이슈, 셸 기록 또는 Git에 복사하지 않는다.

## greetd 전환, ReGreet acceptance와 SDDM rollback
...
grep -F 'Exec=uwsm start -- hyprland.desktop' \
...
잠금만 담당하며 greetd/ReGreet를 대체하지 않는다.
...
ReGreet에서 로그인할 수 없는 경우 `Ctrl+Alt+F2`로 TTY에 들어가 root 권한을
...
SDDM에서 비밀번호, 빈 비밀번호 fingerprint, `Enoshima Hyprland` 세션 선택과
...
card의 너비와 input 높이는 읽기·터치 가능한 상한으로 유지한다. card 높이와 모든
배치는 output 백분율을 사용하므로 `balanced`, `matched`, external-only에서 화면
밖으로 밀려나지 않는다. 정적 모델은 1440x900, 1280x800, 2560x1440,
1920x1080, 1024x768, 800x600 logical output을 검사한다.

실제 장치에서는 다음을 모두 확인한다.

- eDP scale 2.0과 Dell scale 1.5에서 card, input, fingerprint/error text가 잘리지 않음
...
X11/XWayland, 192 DPI와 `XMODIFIERS=@im=fcitx`를 설정한다. 192 DPI는 KakaoTalk가
고정되는 내부 OLED의 2x compositor scale과 일치한다. 숫자가 가장 큰
...
- `balanced`에서 eDP-1은 scale 2.0/1440x900, Dell은 scale 1.5/2560x1440이고
