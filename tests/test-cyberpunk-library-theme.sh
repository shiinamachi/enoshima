...
assert_contains "$hyprlock" 'fractional_scaling = 2'
assert_contains "$hyprlock" 'size = 468, 560'
assert_contains "$hyprlock" 'size = 420, 58'
assert_contains "$hyprlock" 'size = 420, 48'
assert_contains "$hyprlock" 'text = cmd[update:10000] enoshima-auth-status summary'
assert_contains "$hyprlock" "text = ◎  \$FPRINTMESSAGE"
...
assert_contains "$hyprlock" 'rounding = 14'
assert_contains "$hyprlock" 'border_size = 2'
...
assert_contains "$dock" 'readonly property color colorWarning: "#ffb86b"'
...
assert_contains "$dock" '!root.launcherOpen && (!fullscreenActive || manualReveal || keyboardMode)'
...
assert_contains "$dock" 'focusable: keyboardMode'
assert_contains "$dock" 'command: ["cyberdock-pins", "list", "--json"]'
assert_contains "$dock" 'path: root.pinsPath'
assert_contains "$dock" '"id": "unpin", "label": root.tr("dock.unpin")'
assert_contains home/dot_config/enoshima/i18n/en-US.json '"dock.unpin": "Unpin from Dock"'
assert_contains "$dock" '"id": "move-left", "label": root.tr("dock.moveLeft")'
...
assert_contains "$dock" 'readonly property bool urgent:'
assert_contains "$dock" 'readonly property bool keyboardFocused:'
assert_contains "$dock" 'WlrKeyboardFocus.Exclusive'
assert_contains "$dock" 'event.key === Qt.Key_Home'
assert_contains "$dock" 'event.key === Qt.Key_End'
...
assert_contains "$dock" 'id: uiFixtureStateTimer'
assert_contains "$dock" 'blockAllReads: root.uiFixtureEnabled'
...
assert_contains home/dot_config/quickshell/cyberdock/shell.qml 'onLoaded: root.loadUiFixtureState()'
assert_contains home/dot_config/quickshell/cyberdock/shell.qml 'onInternalTextChanged:'
assert_contains home/dot_config/quickshell/cyberdock/shell.qml 'const candidate = parseUiFixtureState(uiFixtureStateFile.text())'
...
assert_contains "$display_overlay" '"id": "mirror", "label": tr("display.mirror")'
assert_contains "$display_overlay" '"label": overlay.tr("display.keep")'
